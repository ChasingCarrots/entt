...
class basic_registry {
    ...
    struct pool_handler: storage<Entity, Component> {
        ...
        sigh<void(basic_registry &, const Entity)> on_update;
        void *group{};
        ...
        decltype(auto) assign(basic_registry &registry, const Entity entt, Args &&... args) {
            if constexpr(std::is_empty_v<Component>) {
                ...
                on_update.publish(registry, entt);
        ...
        decltype(auto) replace(basic_registry &registry, const Entity entt, Args &&... args) {
            if constexpr(std::is_empty_v<Component>) {
                ...
                on_replace.publish(registry, entt, component);
                auto& movedComp = (storage<Entity, Component>::get(entt) = std::move(component));
                on_update.publish(registry, entt);
                return movedComp;
            }
        }

        void update(basic_registry &registry, const Entity entt) {
            on_update.publish(registry, entt);
        }
    };
...
public:
    ...
    decltype(auto) create() {
        ...
        on_entity_created.publish(*this, entity);
    ...
    void destroy(const entity_type entity) {
        ...
        on_entity_destroyed.publish(*this, entity);
    ...
    void update(const entity_type entity) {
        assure<Component>()->on_update.publish(*this, entity);
    }
    ...
    auto on_update() ENTT_NOEXCEPT {
        return assure<Component>()->on_update.sink();
    }
    ...
    auto destruction() ENTT_NOEXCEPT {
        return on_entity_destroyed.sink();
    }
...
private:
    ...
    size_type available{};
    entity_type next{};
    sigh<void(basic_registry &, const Entity)> on_entity_created;
    sigh<void(basic_registry &, const Entity)> on_entity_destroyed;
};
